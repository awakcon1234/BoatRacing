/* Team GUI removed
	...
	if (!inMain && !inTeam && !inMember && !inColorPicker && !inBoatPicker && !inDisbandConfirm && !inLeaveConfirm) return;
		...
		if (inMain && slot == base && p.hasPermission("boatracing.admin")) {
			dev.belikhun.boatracing.BoatRacingPlugin.getInstance().getAdminGUI().openMain(p);
	...
	if (inMain && slot == base + 4) {
			...
			if (!allowCreate2) {
				dev.belikhun.boatracing.util.Text.msg(p, "&cMáy chủ này đã hạn chế việc tạo đội. Chỉ quản trị viên mới có thể tạo đội.");
				p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
				return;
			}
			TeamManager tm = plugin.getTeamManager();
			if (tm.getTeamByMember(p.getUniqueId()).isPresent()) {
				dev.belikhun.boatracing.util.Text.msg(p, "&cBạn đã ở trong một đội.");
		...
		openTeamView(p, target);
			...
			if (it.getType() == Material.PAPER) {
				...
				if (!allowRename && !isAdmin) {
					dev.belikhun.boatracing.util.Text.msg(p, "&cMáy chủ này đã hạn chế việc đổi tên đội. Chỉ quản trị viên mới có thể đổi tên đội.");
					p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
					return;
				}
				if (!isAdmin && !team.isMember(p.getUniqueId())) {
					dev.belikhun.boatracing.util.Text.msg(p, "&cChỉ thành viên của đội mới có thể đổi tên đội của mình.");
					p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
					return;
				}
				...
				if (!allowColor && !isAdmin) {
					dev.belikhun.boatracing.util.Text.msg(p, "&cMáy chủ này đã hạn chế việc đổi màu đội. Chỉ quản trị viên mới có thể đổi màu đội.");
					p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
					return;
				}
				if (!isAdmin && !team.isMember(p.getUniqueId())) {
					dev.belikhun.boatracing.util.Text.msg(p, "&cChỉ thành viên của đội mới có thể đổi màu đội.");
					p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
					return;
				}
				...
				if (team.getMembers().size() >= plugin.getTeamManager().getMaxMembers()) {
					dev.belikhun.boatracing.util.Text.msg(p, "&cĐội này đã đầy.");
					p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
					return;
				}
				...
				dev.belikhun.boatracing.util.Text.msg(p, "&aBạn đã tham gia đội " + team.getName());
				...
				for (java.util.UUID m : team.getMembers()) {
					...
					if (op.isOnline() && op.getPlayer() != null) {
						dev.belikhun.boatracing.util.Text.msg(op.getPlayer(), "&e" + p.getName() + " đã tham gia đội.");
					}
				}
				...
				if (im instanceof SkullMeta) {
					...
					if (sm.getOwningPlayer() != null) {
						...
						if (memberId.equals(p.getUniqueId())) {
							...
							dev.belikhun.boatracing.util.Text.msg(p, "&cChỉ quản trị viên mới có thể quản lý thành viên khác.");
							p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
						}
					}
				}
			...
			plugin.getTeamManager().save();
			dev.belikhun.boatracing.util.Text.msg(p, "&aĐã đặt màu đội thành " + chosen.name() + ".");
			// Notify other team members
			for (java.util.UUID m : team.getMembers()) {
				if (m.equals(p.getUniqueId())) continue;
				org.bukkit.OfflinePlayer op = Bukkit.getOfflinePlayer(m);
				if (op.isOnline() && op.getPlayer() != null) {
					dev.belikhun.boatracing.util.Text.msg(op.getPlayer(), "&e" + p.getName() + " đã đổi màu đội thành " + chosen.name() + ".");
				}
			}
			p.playSound(p.getLocation(), org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 0.8f, 1.3f);
			openTeamView(p, team);
			...
			if (!allowBoat) {
				dev.belikhun.boatracing.util.Text.msg(p, "&cMáy chủ này đã hạn chế việc đổi thuyền. Chỉ quản trị viên mới có thể đặt thuyền của bạn.");
				p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
				return;
			}
			...
			plugin.getTeamManager().save();
			dev.belikhun.boatracing.util.Text.msg(p, "&aĐã đặt loại thuyền thành " + chosen.name() + ".");
			p.playSound(p.getLocation(), org.bukkit.Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 0.8f, 1.3f);
			openMemberProfile(p, team);
			...
			if (it.getType() == Material.RED_CONCRETE && team != null) {
				...
				for (java.util.UUID m : members) {
					...
					if (op.isOnline()) {
						...
						if (mp != null) {
							dev.belikhun.boatracing.util.Text.msg(mp, "&eĐội của bạn đã bị giải tán.");
							mp.playSound(mp.getLocation(), org.bukkit.Sound.ENTITY_VILLAGER_NO, 0.8f, 0.8f);
						}
					}
				}
				plugin.getTeamManager().deleteTeam(team);
				dev.belikhun.boatracing.util.Text.msg(p, "&aĐã giải tán đội.");
			...
			if (it.getType() == Material.RED_CONCRETE) {
				...
				if (team.getMembers().isEmpty()) {
					plugin.getTeamManager().deleteTeam(team);
					dev.belikhun.boatracing.util.Text.msg(p, "&aBạn đã rời và đội của bạn đã bị xóa (không còn thành viên).");
				} else {
					plugin.getTeamManager().save();
					dev.belikhun.boatracing.util.Text.msg(p, "&aBạn đã rời đội.");
					// Notify remaining members
					for (java.util.UUID m : team.getMembers()) {
						org.bukkit.OfflinePlayer op = Bukkit.getOfflinePlayer(m);
						if (op.isOnline() && op.getPlayer() != null) {
							dev.belikhun.boatracing.util.Text.msg(op.getPlayer(), "&e" + p.getName() + " đã rời đội.");
						}
					}
				}
			...
			dev.belikhun.boatracing.util.Text.msg(p, "&cChỉ quản trị viên mới có thể quản lý thành viên. Dùng /boatracing admin team remove <team> <player>.");
			p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
			return;
			...
			Team team = plugin.getTeamManager().getTeams().stream().filter(t -> t.getId().toString().equals(tid)).findFirst().orElse(null);
			if (team == null) return;
			dev.belikhun.boatracing.util.Text.msg(p, "&cHệ thống trưởng nhóm đã bị loại bỏ.");
			p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
			...
			Team team = plugin.getTeamManager().getTeams().stream().filter(t -> t.getId().toString().equals(tid)).findFirst().orElse(null);
			if (team == null) return;
			dev.belikhun.boatracing.util.Text.msg(p, "&cThao tác này chỉ dành cho quản trị viên. Dùng /boatracing admin team remove <team> <player>.");
			p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
	...
	private java.util.List<AnvilGUI.ResponseAction> handleAnvilInput(Player p, String action, UUID teamId, String input) {
		if ("name".equals(action)) {
			...
			if (team == null) { dev.belikhun.boatracing.util.Text.msg(p, "&cKhông tìm thấy đội."); return java.util.Collections.emptyList(); }
			boolean allowRename = plugin.getConfig().getBoolean("player-actions.allow-team-rename", false);
			boolean isAdmin = p.hasPermission("boatracing.admin");
			if (!allowRename && !isAdmin) {
				dev.belikhun.boatracing.util.Text.msg(p, "&cMáy chủ này đã hạn chế việc đổi tên đội. Chỉ quản trị viên mới có thể đổi tên đội.");
			...
			dev.belikhun.boatracing.util.Text.msg(p, "&aĐã đổi tên đội thành &e" + input + "&a.");
			// Notify other team members
			for (java.util.UUID m : team.getMembers()) {
				if (m.equals(p.getUniqueId())) continue;
				org.bukkit.OfflinePlayer op = Bukkit.getOfflinePlayer(m);
				if (op.isOnline() && op.getPlayer() != null) {
					dev.belikhun.boatracing.util.Text.msg(op.getPlayer(), "&e" + p.getName() + " đã đổi tên đội thành &e" + input + "&e.");
				}
			}
			...
			if (!allowNumber) {
				dev.belikhun.boatracing.util.Text.msg(p, "&cMáy chủ này đã hạn chế việc đặt số tay đua. Chỉ quản trị viên mới có thể đặt số của bạn.");
				return java.util.Collections.emptyList();
			}
			...
			dev.belikhun.boatracing.util.Text.msg(p, "&aĐã đặt số tay đua của bạn thành " + num + ".");
