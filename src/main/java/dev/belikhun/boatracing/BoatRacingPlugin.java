...
public class BoatRacingPlugin extends JavaPlugin {
	...
	private static org.bukkit.block.Block getTargetBlockLenient(org.bukkit.entity.Player p, int range) {
		...
		try {
			...
			try {
				org.bukkit.util.RayTraceResult rr = p.rayTraceBlocks((double) range,
		...
		if (command.getName().equalsIgnoreCase("boatracing")) {
			...
			if (args[0].equalsIgnoreCase("scoreboard") || args[0].equalsIgnoreCase("sb")) {
				...
				switch (sub) {
					...
					case "tick" -> {
						try {
							dev.belikhun.boatracing.ui.ScoreboardService svc = scoreboardService;
							if (svc != null)
								svc.forceTick();
					...
					case "debug" -> {
						...
						try {
							scoreboardService.setDebug(enable);
						} catch (Throwable ignored) {
						}
			...
			if (args[0].equalsIgnoreCase("board")) {
				...
				switch (sub) {
					case "status" -> {
						...
						try {
							lines = lobbyBoardService.statusLines();
						} catch (Throwable t) {
			...
			if (args[0].equalsIgnoreCase("race")) {
				if (args.length == 1 || args[1].equalsIgnoreCase("help")) {
					...
					Text.tell(p,
							"&7 - &f/" + label + " race join <track> &7(Tham gia đăng ký cho đường đua; cần có đội)");
					Text.tell(p, "&7 - &f/" + label + " race leave <track> &7(Rời khỏi đăng ký cho đường đua)");
					Text.tell(p,
							"&7 - &f/" + label + " race status <track> &7(Hiển thị trạng thái cuộc đua cho đường đua)");
				...
				switch (args[1].toLowerCase()) {
					...
					case "force" -> {
						...
						if (rm.getRegistered().isEmpty()) {
							Text.msg(p, "&cKhông có người tham gia đã đăng ký. &7Mở đăng ký trước: &f/" + label
									+ " race open");
							p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
							return true;
						}
					...
					case "start" -> {
						...
						java.util.List<org.bukkit.entity.Player> placed = raceManager
								.placeAtStartsWithBoats(participants);
						if (placed.isEmpty()) {
					...
					case "status" -> {
						...
						java.util.List<String> missing = ready ? java.util.Collections.emptyList()
								: tc.missingRequirements();
			...
			if (args[0].equalsIgnoreCase("setup")) {
				...
				if (args.length == 1 || args[1].equalsIgnoreCase("help")) {
					...
					Text.tell(p, "&7 - &f/" + label
							+ " setup addstart &7(Thêm vị trí hiện tại làm vị trí bắt đầu; lặp lại để thêm nhiều)");
					...
					Text.tell(p,
							"&7 - &f/" + label + " setup setbounds &7(Đặt vùng bao đường đua từ selection hiện tại)");
					Text.tell(p, "&7 - &f/" + label + " setup setwaitspawn &7(Đặt điểm spawn chờ từ vị trí hiện tại)");
					Text.tell(p, "&7 - &f/" + label
							+ " setup setfinish &7(Sử dụng selection của bạn để đặt vùng vạch đích)");
					// pit mechanic removed
					Text.tell(p, "&7 - &f/" + label
							+ " setup addcheckpoint &7(Thêm checkpoint từ selection; có thể thêm nhiều. Thứ tự quan trọng)");
					Text.tell(p, "&7 - &f/" + label
							+ " setup addlight &7(Thêm Đèn Redstone đang nhìn thành đèn xuất phát; tối đa 5, từ trái sang phải)");
					...
					Text.tell(p, "&7 - &f/" + label
							+ " setup clearpos <player> &7(Xóa vị trí bắt đầu tùy chỉnh của người chơi)");
				...
				switch (sub) {
					...
					case "setbounds" -> {
						...
						if (sel == null) {
							Text.msg(p, "&cKhông có selection hợp lệ. Dùng &f/" + label + " setup pos1 &7và &f" + label
									+ " setup pos2 &7hoặc dùng wand.");
							p.playSound(p.getLocation(), org.bukkit.Sound.BLOCK_NOTE_BLOCK_BASS, 0.8f, 0.6f);
							return true;
						}
						dev.belikhun.boatracing.track.Region r = new dev.belikhun.boatracing.track.Region(sel.worldName,
								sel.box);
					...
					case "setwaitspawn" -> {
						...
						Text.msg(p, "&aĐã đặt spawn chờ tại &f" + Text.fmtPos(loc) + " &7yaw="
								+ Math.round(loc.getYaw()) + ", pitch=0");
						p.playSound(p.getLocation(), org.bukkit.Sound.UI_TOAST_CHALLENGE_COMPLETE, 0.9f, 1.2f);
						if (setupWizard != null)
							setupWizard.afterAction(p);
						return true;
					...
					case "wand" -> {
						...
						Text.msg(p,
								"&aCông cụ chọn đã sẵn sàng. &7Nhấp trái đánh dấu &fGóc A&7; nhấp phải đánh dấu &fGóc B&7.");
					...
					case "addstart" -> {
						...
						Text.msg(p, "&aĐã thêm vị trí bắt đầu tại &f" + Text.fmtPos(loc) + " &7yaw="
								+ Math.round(loc.getYaw()) + ", pitch=0");
					...
					case "addcheckpoint" -> {
						...
						Text.msg(p, "&aĐã thêm checkpoint #&f" + trackConfig.getCheckpoints().size() + " &7("
								+ Text.fmtArea(r) + ")");
					...
					case "setlaps" -> {
						...
						if (raceService != null)
							raceService.setDefaultLaps(laps);
					...
					case "setpos" -> {
						...
						if (slotArg.equalsIgnoreCase("auto")) {
							...
							if (oneBased < 1 || oneBased > trackConfig.getStarts().size()) {
								Text.msg(p, "&cVị trí không hợp lệ. Phạm vi: 1-" + trackConfig.getStarts().size());
								return true;
							}
							trackConfig.setCustomStartSlot(off.getUniqueId(), oneBased - 1);
							Text.msg(p,
									"&aĐã gán vị trí bắt đầu tùy chỉnh cho &f"
											+ (off.getName() != null ? off.getName() : off.getUniqueId().toString())
					...
					case "show" -> {
						...
						String tname = (getTrackLibrary() != null && getTrackLibrary().getCurrent() != null)
								? getTrackLibrary().getCurrent()
								: "(unsaved)";
						...
						// pit mechanic removed
						Text.tell(p, "&7 - &fVị trí bắt đầu tùy chỉnh: &e"
								+ (customStarts > 0 ? (customStarts + " người") : "không có"));
			...
			if (args.length >= 2 && args[0].equalsIgnoreCase("setup")) {
				...
				if (args.length == 2)
					return List.of("help", "addstart", "clearstarts", "pos1", "pos2", "setbounds", "setwaitspawn",
							"setfinish", "addcheckpoint", "clearcheckpoints", "addlight", "clearlights", "setpos",
							"clearpos", "show", "selinfo", "wand", "wizard");
