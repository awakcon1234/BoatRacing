...
public class SetupWizard {
    ...
    public void start(Player p) {
        ...
        es.jaie55.boatracing.util.Text.msg(p, "&aBắt đầu thiết lập. Trước tiên, chọn tên đường đua: /boatracing setup <name>");
    }

    public void status(Player p) {
        Step s = active.get(p.getUniqueId());
        if (s == null) es.jaie55.boatracing.util.Text.msg(p, "&7Không có phiên thiết lập nào đang hoạt động.");
        else es.jaie55.boatracing.util.Text.msg(p, "&eBước hiện tại: &f" + s.name());
    }

    public void cancel(Player p) {
        active.remove(p.getUniqueId());
        workingName.remove(p.getUniqueId());
        es.jaie55.boatracing.util.Text.msg(p, "&cĐã hủy thiết lập.");
    }
    ...
    public void finish(Player p) {
        ...
        if (name == null || name.isEmpty()) {
            es.jaie55.boatracing.util.Text.msg(p, "&cChưa đặt tên đường đua. Dùng /boatracing setup <name>");
            return;
        }
        boolean ok = trackConfig.save(name);
        if (ok) es.jaie55.boatracing.util.Text.msg(p, "&aĐã lưu đường đua " + name);
        else es.jaie55.boatracing.util.Text.msg(p, "&cLưu đường đua thất bại: " + name);
        active.remove(uid);
    ...
    public void back(Player p) {
        ...
        if (s == null) { es.jaie55.boatracing.util.Text.msg(p, "&7Không có phiên thiết lập nào đang hoạt động."); return; }
        ...
        es.jaie55.boatracing.util.Text.msg(p, "&aĐã quay lại: " + active.get(uid));
    }
    ...
    public void skip(Player p) {
        ...
        if (s == Step.ADD_CHECKPOINTS) {
            ...
            es.jaie55.boatracing.util.Text.msg(p, "&aĐã bỏ qua checkpoint. Tiếp theo: đặt vạch đích.");
        } else es.jaie55.boatracing.util.Text.msg(p, "&7Không có gì để bỏ qua ở đây.");
    }
    ...
    public void next(Player p) {
        ...
        es.jaie55.boatracing.util.Text.msg(p, "&aĐã chuyển sang " + active.get(uid));
    }
    ...
    public void setWorkingName(Player p, String name) {
        ...
        es.jaie55.boatracing.util.Text.msg(p, "&aTên đường đua đang làm việc: &f" + name);
    }
